...
# 누락된 baseline만 다운로드
FIGMA_TOKEN=<토큰> npm run test:visual:update-baseline -- --download

# 전체 baseline 재다운로드
FIGMA_TOKEN=<토큰> npm run test:visual:update-baseline -- --download --all
```
...
| 방법               | 용도                           | 명령어                                                    |
| ------------------ | ------------------------------ | --------------------------------------------------------- |
| Figma에서 다운로드 | 디자인과 구현의 일치를 검증    | `npm run test:visual:update-baseline -- --download`       |
